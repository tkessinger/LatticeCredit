...
## Parallelized implementation of LatticeCredit.
...
function main(args)
    ...
    s = ArgParseSettings(description =
        "run LatticeCredit simulations across multiple cores")
    ...
    defpars = Dict{String,Any}([
        ...
        "focal_type" => Dict("value" => 8, "type" => Int64),
    ...
    @everywhere workers() eval(:(using LatticeCredit))
    ...
    @everywhere function run_worker(inputs, results)
        ...
        while true
            ...
            v = pard["v"]
            κ = pard["κ"]
            focal_type = pard["focal_type"]
            num_trials = pard["num_trials"]
            num_gens = pard["num_gens"]
            gen_cutoff = pard["gen_cutoff"]
            output = pard["output"]

            snapshots = Array{Int64, 2}[]

            println("--- running ", pard["nrun"], " --- ")
            flush(stdout)

            types_to_compete = (focal_type==8) ? collect(0:7) : [focal_type, 7-focal_type]

            game = LatticeGame(r, z, v, κ, 4)
            pop = LatticePopulation(game, N)
            randomize_lattice!(pop, types_to_compete)

            frequencies = zeros(Float64, 8, num_gens - gen_cutoff)

            for j in 1:num_gens
                evolve_sync!(pop)
                if j > gen_cutoff
                    [frequencies[x+1, j - gen_cutoff] = sum(indv==x for indv in pop.lattice)*1.0/N^2 for x in types_to_compete]
                    if (j%50 == 0)
                        push!(snapshots, pop.lattice)
                    end
                end
            end

            mean_freqs = mean(frequencies, dims=2)
            #mean_freqs = [sum(indv==x for indv in pop.lattice)*1.0/N^2 for x in types_to_compete]
            pard["mean_freqs"] = mean_freqs
            pard["snapshots"] = snapshots
    ...
    cols = push!(sort(collect(keys(pars))),
                 ["rep", "mean_freqs", "snapshots", "seed1", "seed2", "seed3", "seed4"]...)
...
main(["--input", "submit/test_parallel_snapshots.json"])
