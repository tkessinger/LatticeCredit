...
r_vals = collect(3:0.3:6) # values of synergy parameter r
v_vals = [0,0.01,0.1,1.0] # loanable amount v
...
for (ri, r) in enumerate(r_vals)
    for (vi, v) in enumerate(v_vals)
        ...
        println("ρ_c at r = $r and v = $v is $(c_frac[ri])")
    end
end
fig = plt.figure()
for (vi, v) in enumerate(v_vals)
